...
import qualified IR
import Utils (fork, assert, assertWith, onFst, onSnd)
import Control.Monad ((>=>))

typecheck :: AST -> IO (Program, Bool)
typecheck ast = return ast
    >>= return . map (onSnd typeblock) . Inst.dict
    >>= foldr (\ (name, blk) io_pair -> putStr (name ++ ": ") >>
        case blk of
        Left  err -> putStrLn (err) >> fmap (onSnd $ const False) io_pair
        Right b@(Block typ is) ->
            if (toPair typ == ([], [])) || name /= "main"
            then putStrLn "Ok!" >> fmap (onFst ((name, b):)) io_pair
            else putStrLn
                ("Main Block should be of type [ -- ], but it's of type "
                ++ show typ) >> fmap (onSnd $ const False) io_pair
        ) (return ([], True))
